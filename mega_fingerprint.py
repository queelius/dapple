...
from dapple.renderers.fingerprint import _get_glyph_cache, _glyph_caches
...
print(f"Building glyph cache (one-time cost)...", flush=True)

_glyph_caches.clear()
t0 = time.monotonic()
cache = _get_glyph_cache(mega, 8, 16)
_ = cache.glyphs
cache_ms = (time.monotonic() - t0) * 1000
print(f"Cache built in {cache_ms:.0f}ms ({len(cache.glyphs)} renderable)")
...
for cat in sorted(categories, key=lambda c: -len(categories[c])):
    ...
    suffix = f"..." if len(chars_list) > 20 else ""
